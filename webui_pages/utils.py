...
class ApiRequest:
    ...
    def list_config_models(
        ...
        '''
        Truy cập danh sách các mô hình được cấu hình trong tệp cấu hình máy chủ, định dạng trả về là {"type": {model_name: config}, ...}.
        '''
        data = {
            "types": types,
        }
    ...
    def stop_llm_model(
        ...
        '''
        Dừng một mô hình LLM cụ thể.
Lưu ý: Do cách thức thực hiện của Fastchat, thực tế là dừng các model_worker chứa mô hình LLM
        '''
        data = {
            "model_name": model_name,
            "controller_address": controller_address,
    ...
    def change_llm_model(
        ...
        def ret_sync():
            ...
            if model_name not in running_models:
                return {
                    "code": 500,
                    "msg": f"Không thể chạy mô hình '{model_name}' được chỉ định. Mô hình đang chạy hiện tại là: {running_models}"
                }
            ...
            if new_model_name not in config_models.get("local", {}):
                return {
                    "code": 500,
                    "msg": f"Cấu hình không có mô hình '{new_model_name}' bạn muốn chuyển đổi."
                }
        ...
        async def ret_async():
            ...
            if model_name not in running_models:
                return {
                    "code": 500,
                    "msg": f"Không thể chạy mô hình '{model_name}' đã chỉ định. Mô hình hiện đang chạy là: {running_models}"
                }
            ...
            if new_model_name not in config_models.get("local", {}):
                return {
                    "code": 500,
                    "msg": f"Không có cấu hình cho mô hình '{new_model_name}' mà bạn muốn chuyển đổi trong tệp cấu hình."
                }
    ...
    def embed_texts(
        ...
        '''
        Việc biểu diễn văn bản dưới dạng vectơ có thể sử dụng các mô hình nhúng cục bộ (local embed_models) và các mô hình nhúng trực tuyến hỗ trợ (online embeddings).
        '''
        data = {
            "texts": texts,
            "embed_model": embed_model,
    ...
    def chat_feedback(
        ...
        '''
        Đánh giá phản hồi cuộc trò chuyện
        '''
        data = {
